...
ordersAdminRouter.post('/', async (c) => {
  ...
  for (const item of body.items) {
    ...
    const dbProduct = await getDocument('products', item.productId);
    if (!dbProduct) {
...
ordersAdminRouter.patch('/:id/status', async (c) => {
  ...
  const updated = await setDocument('orders', id, { ...existing, status: body.status });
...
ordersAdminRouter.delete('/:id', async (c) => {
  const id = c.req.param('id');
  const deleted = await deleteDocument('orders', id);
  if (!deleted) {
    return c.json({ success: false, message: 'Order not found' }, 404);
  }
